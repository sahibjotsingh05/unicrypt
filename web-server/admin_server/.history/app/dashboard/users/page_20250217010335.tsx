...
export default function UsersPage() {
	...
	const handleUpdateStudentId = async (username: string, newId: string) => {
		...
		try {
			...
			setUsers((prevUsers) => {
				console.log(`🔄 Updating local state for ${username}`);
	...
	const handleUpdateAccess = async (
		...
		try {
			...
			setUsers((prevUsers) => {
				console.log(`🔄 Updating local state for ${username} with:`, updates);
				return prevUsers.map((user) =>
					user.username === username ? { ...user, ...updates, username } : user
				);
			});
